...
async def receive_chat_id(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("❌ Could not add this chat. Check the chat ID and ensure the bot is an admin.")
...
@dp.message(AdminState.waiting_chat_id)
async def remove_or_add_chat(message: Message, state: FSMContext):
    # This handler is intentionally kept compatible with the simple original flow.
    # If the chat exists, remove it; otherwise add it.
    if not await is_admin(message.from_user.id):
        return
    try:
        chat_id = int(message.text.strip())
        ...
        if any(c["chat_id"] == chat_id for c in chats):
            await remove_force_join(chat_id)
            await state.clear()
        ...
        await message.answer("❌ Invalid chat ID or bot lacks required admin permissions.")
